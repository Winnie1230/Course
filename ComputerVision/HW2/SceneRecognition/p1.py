...
parser.add_argument('--feature', help='feature',
                    type=str, default='dumy_feature')
parser.add_argument('--classifier', help='classifier',
                    type=str, default='dumy_classifier')
parser.add_argument('--dataset_path', help='dataset path',
                    type=str, default='../hw2_data/p1/')
...
def main():
    ...
    if FEATURE == 'tiny_image':
        ...
        vocab_size = 400
        vocab_file = 'vocab_' + str(vocab_size) + '.pkl'
        train_feats_file = 'train_image_feats_' + str(vocab_size) + '.pkl'
        test_feats_file = 'test_image_feats_' + str(vocab_size) + '.pkl'
        if os.path.isfile(vocab_file) is False:
            print(
        ...
        if os.path.isfile(train_feats_file) is False:
            # TODO Modify get_bags_of_sifts.py
            print("create train_image_feats")
            train_image_feats = get_bags_of_sifts(
                train_image_paths, vocab_file)
            # ----- save to pickle file -----
            with open(train_feats_file, 'wb') as handle:
                pickle.dump(train_image_feats, handle,
                            protocol=pickle.HIGHEST_PROTOCOL)
        else:
            print("exist train_feats_file")
            with open(train_feats_file, 'rb') as handle:
                train_image_feats = pickle.load(handle)

        if os.path.isfile(test_feats_file) is False:
            print("create test_image_feats")
            test_image_feats = get_bags_of_sifts(test_image_paths, vocab_file)
            # ----- save to pickle file -----
            with open(test_feats_file, 'wb') as handle:
                pickle.dump(test_image_feats, handle,
                            protocol=pickle.HIGHEST_PROTOCOL)
        else:
            print("exist test_feats_file")
            with open(test_feats_file, 'rb') as handle:
    ...
    accuracy = float(len([x for x in zip(
        test_labels, predicted_categories) if x[0] == x[1]]))/float(len(test_labels))
    ...
    print(len(predicted_categories_ids))
    train_labels_ids = [CATE2ID[x] for x in train_labels]
    print(len(train_labels_ids))

    # Step 3: Build a confusion matrix and score the recognition system
    # You do not need to code anything in this section.

    build_confusion_mtx(
        test_labels_ids, predicted_categories_ids, ABBR_CATEGORIES)
